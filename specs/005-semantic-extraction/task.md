...
- [x] **프롬프트 및 파서 설계**
    - [ ] `app/domain/prompts/substruction.py`: 엔티티 추출을 위한 프롬프트 템플릿 작성 (Skipped - Inline in Extractor)
...
- [ ] **Ingestion Service 연동**
    - [ ] `IngestionService.process_job` 흐름에 `SemanticeExtractor` 단계 추가
    - [ ] 추출된 메타데이터를 Document와 함께 저장 (DB 스키마 변경 필요 시 반영)

## 4. 검증 (Verification)
- [ ] **단위 테스트**
    - [ ] LLM 응답 Mocking 하여 추출 로직 테스트 (`tests/domain/test_extractor.py`)
- [ ] **통합 테스트**
    - [ ] 실제 웹 수집 후 메타데이터가 정상적으로 추출/저장되는지 확인
