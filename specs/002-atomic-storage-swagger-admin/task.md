...
- [x] Interface Layer (API) 수정
  - [x] `POST /ingest/web`에서 저장 옵션 처리
  - [x] `GET /documents` 엔드포인트 추가 (저장된 문서 목록 조회)
- [x] 문서화 (Documentation)
  - [x] `docs/architecture.md`: DDD 구조(Entity/VO/Storage) 반영 및 설계 근거 기술
- [x] 테스트 및 검증
  - [x] DB 연동 통합 테스트 (Test Containers 또는 Local Docker)
