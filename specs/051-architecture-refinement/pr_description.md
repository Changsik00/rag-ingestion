...
- **API 경로 통합 및 표준화**:
  - 모든 엔드포인트를 `/v1/` 하위로 통합 (예: `/v1/ingest/web`)
  - 모든 경로에서 의존적인 `/admin/` 접두어 제거
  - `app/interfaces/api/v1/endpoints/`로 라우터 관리 일원화
- **의존성 주입(DI) 최적화**:
  - `get_repository()` 시그니처 정제 및 `lru_cache` 효율화
  - `docker-compose.yml` 내 `ADMIN_API_URL` 환경 변수 동기화
...
```
**테스트 결과 요약:**
- ✅ 194 passed, 60 skipped
- ⚠️ Warnings: 47 (로직에 영향 없는 외부 라이브러리 경고)
