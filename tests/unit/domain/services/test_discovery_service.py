...
def mock_search_client():
    ...
    client.search.return_value = [
        SearchResult(title="Test", link="http://example.com", snippet="Test")
    ]
    return client
...
async def test_start_discovery_flow(discovery_service, mock_search_client, mock_ingestion_service):
    ...
    discovery_service._fetch_links = AsyncMock(return_value=[
...
async def test_blocklist(discovery_service):
    assert discovery_service._is_blocked("http://youtube.com/watch?v=123") == True
    assert discovery_service._is_blocked("http://example.com/file.pdf") == True
    assert discovery_service._is_blocked("http://good-site.com/article") == False

@pytest.mark.asyncio
async def test_max_docs_limit(discovery_service, mock_ingestion_service):
    discovery_service._fetch_links = AsyncMock(return_value=[
        f"http://example.com/{i}" for i in range(20)
    ])
