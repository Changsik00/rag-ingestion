...
def test_graph_builder_import():
    if not importlib.util.find_spec("app.infrastructure.ai.ingest.graph_builder"):
        pytest.fail("app.infrastructure.ai.ingest.graph_builder module not found")

    try:
        from app.infrastructure.ai.ingest.graph_builder import IngestionGraphBuilder  # noqa: F401
    except ImportError:
        pytest.fail("IngestionGraphBuilder not found in app.infrastructure.ai.ingest.graph_builder")
