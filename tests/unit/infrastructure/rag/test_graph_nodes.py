...
def test_nodes_module_import():
    if not importlib.util.find_spec("app.infrastructure.ai.ingest.nodes"):
        pytest.fail("app.infrastructure.ai.ingest.nodes module not found")

    # Try importing class
    try:
        from app.infrastructure.ai.ingest.nodes import IngestionNodes  # noqa: F401
    except ImportError:
        pytest.fail("IngestionNodes class not found in app.infrastructure.ai.ingest.nodes")
