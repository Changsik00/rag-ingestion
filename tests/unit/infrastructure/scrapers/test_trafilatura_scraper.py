...
def test_scrape_clean_extraction(scraper):
    ...
    with patch("trafilatura.fetch_url", return_value=MOCK_HTML_WITH_ADS), \
...
def test_scrape_fallback_when_trafilatura_fails(scraper):
    ...
    with patch("trafilatura.fetch_url", return_value="<html>Text</html>"), \
        ...
        try:
            response = scraper.scrape("https://example.com/fail")
            # Fallback 구현 시:
