...
async def test_session_creation_and_cleanup():
    ...
    with TestClient(app) as client:
        ...
        response = client.post(f"/v1/rag/sessions/{thread_id}/ask", json=payload)
        # 202 or 200 acceptable
        assert response.status_code in [200, 202]
        
        # 2. Verify Data Exists
        # Verify via Trace endpoint
        res_trace = client.get(f"/v1/rag/sessions/{thread_id}/trace")
        assert res_trace.status_code == 200
        
        # 3. Call Reset (Cleanup)
        res_reset = client.post(f"/v1/rag/sessions/{thread_id}/reset")
        assert res_reset.status_code == 200
        # Expect successful message (will fail initially as "Not Supported")
        assert "successfully" in res_reset.json()["message"]
        
        # 4. Verify Deletion
        res_trace_after = client.get(f"/v1/rag/sessions/{thread_id}/trace")
