...
def chroma_repo():
    ...
    # Setup: 테스트 전 컬렉션 리셋 (이전 테스트 데이터 제거)
    repo.reset_collection()
    yield repo
    # Teardown: 테스트 후 컬렉션 리셋
    repo.reset_collection()
...
def test_duplicate_document_upsert(chroma_repo):
    ...
    doc_v1 = Document(
        ...
        metadata={"source_id": "https://example.com/test", "title": "Test Document v1"},
    )
    ...
    doc_v2 = Document(
        ...
        metadata={"source_id": "https://example.com/test", "title": "Test Document v2"},
    )
...
def test_duplicate_chunks_upsert(chroma_repo):
    ...
    chunks_v1 = [
        Chunk(id=chunk_id, content="Initial chunk content", parent_id=doc_id, index=0, metadata={"title": "Chunk v1"})
    ]
    ...
    # When: 동일 ID로 2차 수집 (내용 변경)
    chunks_v2 = [
        Chunk(id=chunk_id, content="Updated chunk content", parent_id=doc_id, index=0, metadata={"title": "Chunk v2"})
    ]
...
def test_batch_chunks_upsert(chroma_repo):
    ...
    chunks = [
        Chunk(
            id=str(uuid4()), content=f"Chunk {i} content", parent_id=doc_id, index=i, metadata={"title": f"Chunk {i}"}
        )
        for i in range(10)
    ]
