...
def test_scenario_1_to_4_entity_relationships_flow(sample_document_with_relationships):
    ...
    mock_content = IngestResponse(
        ...
        metadata={"title": "Elon Musk Bio", "source_id": sample_document_with_relationships["url"]},
        message="Mock scrape success"
    )
    # Must use AsyncMock for async method
    mock_scraper.scrape = AsyncMock(return_value=mock_content)
    ...
    try:
        # When: 문서 수집 요청
        response = client.post("/v1/ingest/web", json={"url": sample_document_with_relationships["url"], "enable_extraction": True})
        ...
        for _ in range(max_retries):
            ...
            if status_response.status_code == 200:
                job_resp = status_response.json()
                job_status = job_resp.get("current_status") or job_resp.get("status")
                if job_status in ["COMPLETED", "FAILED"]:
                    # Preserve Error Message
                    if job_status == "FAILED":
                         raise AssertionError(f"Job failed: {job_resp.get('error_message')}")
                    break
            sleep(0.5)
    ...
    response = client.get(f"/v1/entities/{entity_name}/relationships")
    ...
    response = client.get(f"/v1/entities/{entity_name}/relationships", params={"relationship_type": "FOUNDED"})
    ...
    response = client.get(f"/v1/entities/{entity_name}/relationships", params={"relationship_type": "INVALID_TYPE"})
