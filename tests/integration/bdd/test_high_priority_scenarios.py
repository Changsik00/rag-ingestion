...
def test_invalid_job_id_returns_404():
    ...
    assert "not found" in error_detail.lower(), \
        f"Error message should contain 'not found', but got: {error_detail}"
    assert non_existent_job_id in error_detail, \
        f"Error message should contain job_id, but got: {error_detail}"
...
def test_duplicate_url_sequential_ingestion():
    ...
    assert job1["status"] == "COMPLETED", \
        f"Job 1 should be COMPLETED but got {job1['status']}"
    assert job2["status"] == "COMPLETED", \
        f"Job 2 should be COMPLETED but got {job2['status']}"
    ...
    assert len(set(doc_ids)) == len(doc_ids), \
        "All document IDs should be unique (no duplicates)"
