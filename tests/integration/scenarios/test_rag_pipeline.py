...
class TestRagPipelineScenarios:
    ...
    async def test_reasoning_strategy_selection(self):
        # Given: A validation failure state
        ...
        feedback = [ValidationFeedback(source="validator", message="err",
                                       target_fields=["title"])]
    ...
    async def test_intent_classifier_contextual_history(self, mock_llm_intent):
        """
        Scenario: Intent classification with conversation history (Spec-012)
        """
        classifier = IntentClassifier(mock_llm_intent)
        ...
        assert intent.intent == IntentType.COMPARE
        # The mock LLM logic in test_rag_pipeline already supports this
    ...
    async def test_intent_classifier_summarize_with_pronoun(self, mock_llm_intent):
        """
        Scenario: Summarize 'this' document mentioned in history
        """
        classifier = IntentClassifier(mock_llm_intent)
