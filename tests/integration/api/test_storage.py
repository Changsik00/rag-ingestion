...
def test_get_storage_stats():
    """GET /v1/storage/stats 엔드포인트가 정상적으로 정보를 반환하는지 테스트"""
...
def test_get_document_reports():
    """GET /v1/storage/reports 엔드포인트 테스트"""
    client.get("/v1/storage/reports")
    # 현재 __init__.py 에서 router.include_router(storage.router, prefix="/storage")
    # 그리고 app.include_router(admin_router, prefix="/v1")
    # 따라서 엔드포인트는 /v1/storage/stats 이런 식이 되어야 함
    pass
...
def test_storage_api_endpoints_exist():
    ...
    # Diagnostic
    response = client.get("/v1/storage/documents/test_id/diagnostic")
    assert response.status_code == 200
