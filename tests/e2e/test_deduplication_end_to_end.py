...
class TestDeduplicationEndToEnd:
    ...
    async def test_duplicate_job_is_skipped(self):
        ...
        # Assertions
        assert job2_final.status == JobStatus.SKIPPED, \
            f"Second job should be SKIPPED, got {job2_final.status}"
        assert job2_final.skip_reason is not None, \
    ...
    async def test_force_refresh_bypasses_deduplication(self):
        ...
        job2_final = self.job_repository.get_job(job2.job_id)
        assert job2_final.status == JobStatus.SKIPPED, \
            "Second job without force_refresh should be SKIPPED"
        ...
        assert job3_final.content_hash is not None, \
            "Content hash should be calculated for force-refreshed job"
