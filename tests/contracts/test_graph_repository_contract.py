...
def test_graph_repository_has_required_methods():
    ...
    protocol_methods = {name for name in dir(GraphRepository) if not name.startswith("_")}

    assert required_methods.issubset(protocol_methods), f"Missing methods: {required_methods - protocol_methods}"
...
def test_save_entity_signature():
    ...
    assert hints["return"] == str  # noqa: E721
...
def test_get_entities_by_document_signature():
    ...
    assert hints["doc_id"] == str  # noqa: E721
...
def test_neo4j_graph_repository_implements_protocol():
    ...
    impl_methods = {
        name
        for name in dir(Neo4jGraphRepository)
