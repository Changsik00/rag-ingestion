...
async def health_check():
    ...
    return SystemStatusResponse(version="1.0.0", uptime=0.0, components={"api": "ok", "db": "ok"})
...
async def list_documents(repository: Annotated[DocumentRepository, Depends(get_repository)], limit: int = 10):
    ...
    return [DocumentDTO(id=str(d.id), content=d.content, metadata=d.metadata.model_dump(), score=None) for d in docs]
