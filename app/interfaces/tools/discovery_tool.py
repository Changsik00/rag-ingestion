    ...
    async def _arun(self, topic: str, max_depth: int = 1, max_docs: int = 5) -> str:
        try:
            ...
            ingestion_service = Ingestion(
                scraper=scraper, repository=repo, graph=graph, job_repository=job_repo, chunker=chunker, extractor=None
            )

            search_client = get_google_search_client()

            discovery_service = DiscoveryService(search_client=search_client, ingestion_service=ingestion_service)

            # 2. Execute Logic
            job_ids = await discovery_service.start_discovery(topic=topic, max_depth=max_depth, max_docs=max_docs)
