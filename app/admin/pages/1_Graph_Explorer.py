...
with col1:
    ...
    # 3. Raw Cypher
    cypher_input = st.text_area("SQL (Cypher)", value=st.session_state.get('cypher_query', "MATCH (n) RETURN n LIMIT 25"), height=150)
        ...
        try:
            with st.spinner("Fetching Graph Data..."):
                ...
                for n in node_data:
                    ...
                    color = "#97C2FC"
                    if "Person" in n['labels']: color = "#FF7675"
                    elif "Organization" in n['labels']: color = "#74B9FF"
                    elif "Technology" in n['labels']: color = "#55E6C1"
                    elif "Document" in n['labels']: color = "#A29BFE"
                    elif "Chunk" in n['labels']: color = "#DFE6E9"
