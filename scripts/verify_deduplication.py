...
def check_server():
    try:
        # Assuming there's a health endpoint or just checking root
        # Using a known endpoint like /jobs to check connectivity
...
def verify_web_deduplication():
    ...
    # 1. First Ingestion
    print(f"1. Requesting ingestion for: {url}")
    payload = {
        "url": url,
        "force_refresh": False,
        "force_refresh": False,
    ...
    # Check Job 2 Status immediately or after a short wait
    print("   -> Waiting 2 seconds for Job 2 to be processed...")
    time.sleep(2)
    
    j2_info = requests.get(f"{BASE_URL}/jobs/{job2_id}").json()
    j2_status = j2_info["status"]
    j2_msg = j2_info.get("error_message", "")
    
    print(f"   -> Job 2 Status: {j2_status} (Msg: {j2_msg})")
    
    # IngestionJob status used 'SKIPPED' if duplicated.
    if j2_status == "SKIPPED":
        print("✅ SUCCESS: Job 2 was SKIPPED as duplicate.")
    elif j1_status == "COMPLETED" and j2_status == "COMPLETED":
        # Maybe it was too fast or something? But usually should be SKIPPED
        print("❓ Job 2 finished as COMPLETED. Check if it was actually skipped or re-processed.")
    else:
...
def verify_file_deduplication():
    ...
    # Reset file pointer or recreate list because requests might consume it? 
    # Usually easier to recreate list
