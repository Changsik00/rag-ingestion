...
def check_extraction():
    ...
    with driver.session() as session:
        ...
        result = session.run("MATCH (j:IngestionJob) WHERE j.raw_content IS NOT NULL RETURN j.job_id, j.raw_content, j.filename ORDER BY j.created_at DESC LIMIT 1")
        ...
        try:
            ...
            result = session.run("MATCH (d:Document) RETURN d.id, left(d.content, 100) as content ORDER BY d.created_at DESC LIMIT 3")
            for rec in result:
