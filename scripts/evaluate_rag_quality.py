...
async def test_config(name, top_k, temperature, query="일론 머스크에 대해서 알려줘"):
    ...
    payload = {
        ...
        "filters": {},
        "hitl_enabled": False,
        "advanced_settings": {"top_k": top_k, "temperature": temperature, "search_strategy": "hybrid"},
    }
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            ...
            if result.get("messages"):
                ...
                answer = next(
                    (m["content"] for m in reversed(result["messages"]) if m["role"] in ["ai", "assistant"]),
                    "No response",
                )
...
async def main():
    ...
    for s in summary:
        print(
            f"[{s['name']}] Rewritten: {s['rewritten']} | Context: V:{s['retrieval']['vector']}, K:{s['retrieval']['keyword']} | Citations: {s['citations']}"
        )
