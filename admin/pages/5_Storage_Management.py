...
try:
    ...
    if stats:
        ...
        st.error("Failed to fetch system stats from API.")
                ...
                with tab1:
                    if row["status"] == "Missing Title":
                        ...
                        with st.spinner("Loading snippet..."):
                            diag = api_client.get(f"/storage/documents/{doc_id}/diagnostic")
                            snippet = diag.get("snippet") if diag else None
        ...
        if st.button(button_label, type=button_type, disabled=not needs_sync):
            with st.status("Performing Full Synchronization...", expanded=True) as status:
                ...
                if res and res.get("success"):
                    ...
                    st.error("Failed to perform global sync.")
    else:
        st.info("No reports available for global recovery.")
except Exception as e:
